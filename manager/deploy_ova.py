...
def main():
    ...
    if args.vm_name:
        cisp.entityName = args.vm_name  # This tells VMware: "Ignore the OVA name, use this"
...
class OvfHandler(object):
    ...
    def upload_disk(self, file_item, lease, host):
        ...
        """
        # --- REPLACED LOGIC ---
        # Look for the device URL matching this file item
        device_url = None
        for u in lease.info.deviceUrl:
        ...
        # Skip if it's not a disk (like nvram)
        if not device_url.disk:
            print(f"Skipping non-disk file: {file_item.path}")
            return
        # ----------------------
